...
function skai_recovery_post(WP_REST_Request $request) {
    $body = $request->get_json_params(); if (!is_array($body) || !isset($body['action']) || !in_array($body['action'], ['install_contract','dry_run','execute'], true)) return new WP_Error('streetkingz_ai_recovery_request_invalid', 'Only install_contract, dry_run, or execute are supported.', ['status' => 400]);
    ...
    if (!skai_recovery_exact_keys($body, ['action'])) return new WP_Error('streetkingz_ai_recovery_request_invalid', 'Dry-run/execute accepts no caller-supplied target.', ['status' => 400]);
    ...
    if ($body['action'] === 'dry_run') return rest_ensure_response(['status' => 'dry_run_passed','incident' => SKAI_RECOVERY_INCIDENT,'template_id' => 2003,'meta_key' => SKAI_RECOVERY_META_KEY,'observed_current_sha256' => SKAI_RECOVERY_CURRENT_HASH,'target_sha256' => SKAI_RECOVERY_TARGET_HASH,'source_length' => $preflight['current_length'],'target_length' => $preflight['target_length'],'differing_paths' => 140,'expected_numeric_string_reversions' => 140,'unexpected_differences' => 0,'product_70_protected' => true,'verification' => 'fresh_authoritative_raw_hash_and_strict_parsed_equality','content_writes' => 0,'claims' => 0]);
