...
Version 1.2.0 additively exposes a bounded `woocommerce` response section using WooCommerce public product getters. It includes identity, prices and currency, bounded inventory, assigned product categories, structured attributes, at most 100 bounded variations, stored upsell/cross-sell IDs, and media IDs. It never exposes arbitrary post meta. Dynamically derived WooCommerce related-product recommendations are deliberately omitted because they are not stable stored product relationships. Authentication, the custom read-only capability, GET-only registration, fixed Elementor template checks, and route-scoped no-cache controls are unchanged.
