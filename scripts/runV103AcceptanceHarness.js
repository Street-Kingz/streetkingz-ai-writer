...
let product;
const waitFor = (path, expectedStatus) => new Promise((resolve, reject) => { const deadline = Date.now() + 15000; const req = () => { if (Date.now() > deadline) return reject(new Error(`Product readiness check failed for ${path}.`)); if (product.exitCode !== null || product.signalCode) return reject(new Error("Product process exited before readiness checks completed.")); const r = http.get({ hostname: "127.0.0.1", port, path, headers: { host: `127.0.0.1:${port}` } }, response => { response.resume(); if (response.statusCode === expectedStatus) return resolve(); setTimeout(req, 100); }); r.on("error", () => setTimeout(req, 100)); }; req(); });
try { await originPromise; product = spawn(process.execPath, ["index.js"], { env: { ...baseEnv, PRODUCT_PUBLIC_ORIGIN: origin }, stdio: "ignore" }); product.exitCode = null; product.once("exit", (code, signal) => { product.exitCode = code; product.signalCode = signal; }); await waitFor("/health", 200); await waitFor("/internal/v1-03", 200); console.log("V1-03 Acceptance Harness ready"); console.log(`Local UI: http://127.0.0.1:${port}/internal/v1-03`); console.log(`Public Product callback origin: ${origin}`); console.log("Woo app name: V1-03 Street Kingz Acceptance"); console.log("Supabase: local"); } catch (error) { console.error(error.message); tunnel.kill("SIGTERM"); if (product) product.kill("SIGTERM"); console.error("Manual fallback: start a HTTPS tunnel to the local Product port, set PRODUCT_PUBLIC_ORIGIN=https://<origin>, then run V1_03_ACCEPTANCE_HARNESS=1 npm start."); process.exit(1); }
