...
Persist submitted URL, canonical HTTPS origin, authenticated store identity facts and source/version. Normalise trailing slash; preserve supported WordPress base paths/subdirectories; require the exact canonical HTTPS hostname. Host-changing redirects, including www/non-www, are not followed with WooCommerce credentials; the customer must supply the actual canonical WooCommerce host. A materially different host fails closed and requires correction.
