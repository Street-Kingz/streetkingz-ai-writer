...
- Route-only synthetic suite: 15 passed, 0 failed, including numeric `key_id`, charset content type, invalid legacy connected-state rejection, expired-attempt short-circuit, no-store/referrer headers, callback-only acknowledgement, return-triggered verification, and stale consumed-token handling.
- Maintenance unit suite: 2 passed, 0 failed; cleanup runs at startup and every five minutes through an unref timer and tolerates missing Product-kernel configuration.
- Correction migration: generic customer transitions cannot manage Woo provider state; connected Woo rows require granted consent, a Vault reference, and an established Commerce Store. Completion/disconnect/failure share a Connection-scoped advisory lock; disconnect cancels live attempts and completion of an already-consumed valid attempt is idempotent.
