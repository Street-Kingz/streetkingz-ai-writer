...
export function canonicalAuthoritativePost(record, provenance) {
  if (record?.schema_version === 2 && record?.product && record?.elementor_template) {
    ...
    return { schema_version: "2.0.0", artifact_type: "wordpress_authoritative_post_and_template_read", post_id: Number(record.product.id), post_type: record.product.post_type, status: String(record.product.post_status ?? ""), fields, ...(record.woocommerce ? { woocommerce: record.woocommerce } : {}),
