...
router.get("/api/product/connections", handle(async (req, res) => {
  ...
  const { data, error } = await client.from("connections").select("id,business_id,provider_type,status,consent_state,secret_reference,connected_at,disconnected_at,last_success_at,safe_error_code,safe_error_message,created_at,updated_at").in("business_id", (await client.from("businesses").select("id").eq("account_id", account.id)).data?.map(x => x.id) || []); if (error) throw error; res.json({ connections: data || [] });
}));
...
router.patch("/api/product/connections/:id", handle(async (req, res) => {
  const { client, account } = await authContext(req); if (!account) throw new ProductError("TENANT_NOT_FOUND", "Product account is not provisioned.", 404);
  const { data: current, error: readError } = await client.from("connections").select("id,business_id,status,consent_state").eq("id", req.params.id).maybeSingle(); if (readError) throw readError; if (!current) throw new ProductError("CONNECTION_NOT_FOUND", "Connection not found.", 404);
  const next = req.body?.status || current.status; if (next !== current.status) assertConnectionTransition(current.status, next); const consent = req.body?.consent_state || current.consent_state; assertConsentState(consent);
  const patch = { status: next, consent_state: consent, ...(next === "disconnected" ? { disconnected_at: new Date().toISOString(), secret_reference: null } : {}) }; const { data, error } = await client.from("connections").update(patch).eq("id", current.id).select("id,business_id,provider_type,status,consent_state,secret_reference,connected_at,disconnected_at,last_success_at,created_at,updated_at").single(); if (error) throw error; await audit({ account, businessId: current.business_id, eventType: next === "disconnected" ? "connection_disconnected" : "connection_status_changed", correlationId: req.correlationId }); res.json({ connection: data });
}));
