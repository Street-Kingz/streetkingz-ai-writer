...
router.get("/api/product/organic-evidence/search-console/callback", handle(async (req, res) => {
  ...
  const fail = async code => { await admin.rpc("gsc_fail_oauth_attempt", { p_attempt_id: attempt.attempt_id, p_code: code }); throw new ProductError(code, "Google Search Console authorization could not be completed.", 409); };
...
router.post("/api/product/organic-evidence/search-console/select", handle(async (req, res) => {
  ...
  const activated = await admin.rpc("gsc_activate_property", { p_attempt_id: authorization.attemptId, p_site_url: property.siteUrl, p_property_type: property.type, p_permission_level: verified.permissionLevel }); if (activated.error) { if (activated.error.message?.includes("GSC_ACTIVATION_INVALID") || activated.error.message?.includes("CONNECTION_NOT_FOUND")) throw new ProductError("GSC_REAUTH_REQUIRED", "Search Console authorization expired; reconnect to continue.", 409); throw activated.error; } if (!activated.data) throw new ProductError("GSC_REAUTH_REQUIRED", "Search Console authorization expired; reconnect to continue.", 409);
...
router.post("/api/product/organic-evidence/search-console/disconnect", handle(async (req, res) => { const { business, admin } = await context(req); const result = await admin.rpc("gsc_disconnect", { p_business_id: business.id, p_connection_id: req.body?.connection_id }); if (result.error) throw result.error; res.json({ status: "disconnected", consent_state: "revoked", local_credential_removed: true, remote_google_revocation_requested: false }); }));
