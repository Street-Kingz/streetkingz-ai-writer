...
export function createWooCommerceRouter(overrides = {}) {
 ...
 router.post("/api/product/woocommerce/sync", REQUEST_PARSER, handle(async function (req, res) {
  ...
  try {
   ...
   const snapshot = syncMode === "incremental" ? await deps.collectIncrementalCommerce(provider, { syncStartedAt: startedAt, previousSuccessfulAt: established.store.last_successful_at || null, current, perPage: deps.initialSyncPerPage || 100 }) : await deps.collectInitialCommerce(provider, { syncStartedAt: startedAt, perPage: deps.initialSyncPerPage || 100 });
