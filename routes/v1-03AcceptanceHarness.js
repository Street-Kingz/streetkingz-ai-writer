...
export async function readCurrentProductSnapshot({ admin, client, accountId, connectionId }) {
  ...
  const productRows = await admin.from("commerce_products").select("id,source_id").eq("store_id", store.id).eq("generation_id", generation.id); if (productRows.error) throw productRows.error;
  const categoryRows = await admin.from("commerce_categories").select("id,source_id").eq("store_id", store.id).eq("generation_id", generation.id); if (categoryRows.error) throw categoryRows.error;
  const productIds = new Map((productRows.data || []).map(row => [row.id, row.source_id])), categoryIds = new Map((categoryRows.data || []).map(row => [row.id, row.source_id]));
  const linkRows = await admin.from("commerce_product_categories").select("product_id,category_id").eq("store_id", store.id).eq("generation_id", generation.id); if (linkRows.error) throw linkRows.error;
  const links = (linkRows.data || []).map(row => ({ product_source_id: productIds.get(row.product_id), category_source_id: categoryIds.get(row.category_id) }));
  if (links.some(row => !row.product_source_id || !row.category_source_id)) throw new ProductError("CURRENT_GENERATION_INVALID", "Current commerce generation contains an invalid Product-category link.", 502);
  counts.links = links.length;
  return { connection: { state: connection.data.status, consent: connection.data.consent_state }, store: { sync_state: store.sync_state }, generation: { id: generation.id, state: generation.state }, counts, rows: rows.data || [], links };
}
