  ...
  try {
    ...
    const result = await evaluateCandidates({ candidates: candidates.data, packet: evidence.packet, interpretationProvider: provider,
      resolveBatch: async ({ batch, batchIndex, inputHash, request }) => {
        ...
        if (found.error) throw found.error;
        if (found.data.state === "complete") { const saved = await admin.from("organic_candidate_evaluations").select("*").eq("evaluation_run_id", evalRun.id).in("candidate_id", batch.map(c => c.candidate_id)); if (saved.error) throw saved.error; if ((saved.data || []).length === batch.length) return { reused: true, response: { provider: found.data.provider, model: found.data.model, response_id: found.data.response_id, usage: { prompt_tokens: found.data.input_tokens, completion_tokens: found.data.output_tokens }, output: saved.data.map(persistedEvaluationToProviderOutput) } }; }
        if (ownedBatchClaims.get(batchIndex)?.claimToken === found.data.claim_token) return null;
        if (process.env.V105_RECOVERY_MODE === "1" && typeof globalThis.__V105_RECOVERY_PREFLIGHT === "function") globalThis.__V105_RECOVERY_PREFLIGHT(provider.requestPayload({ systemPrompt: buildInterpretationSystemPrompt(), userPrompt: JSON.stringify(request), responseSchema: INTERPRETATION_RESPONSE_SCHEMA, schemaName: "organic_candidate_interpretation", maxOutputTokens: MAX_CALL_OUTPUT_TOKENS }));
        const claimToken = randomUUID(); const started = await admin.rpc("start_candidate_interpretation_attempt", { p_batch_id: found.data.id, p_claim_token: claimToken, p_timeout_seconds: 300 }); if (started.error) throw started.error; if (started.data !== "started") { if (started.data === "provider_outcome_unknown") { const error = new Error("PROVIDER_OUTCOME_UNKNOWN"); error.code = "PROVIDER_OUTCOME_UNKNOWN"; throw error; } if (started.data === "complete") return { pending: true }; if (["output_budget_exceeded", "attempt_bound_exceeded", "retry_exhausted"].includes(started.data)) { const error = new Error(`INTERPRETATION_${String(started.data).toUpperCase()}`); error.code = "INTERPRETATION_CALL_BOUND_EXCEEDED"; throw error; } return { pending: true }; } ownedBatchClaims.set(batchIndex, { claimToken, batchId: found.data.id, startedAt: Date.now() }); return null;
      },
      onBatchComplete: async ({ batchIndex, inputHash, response, rows }) => {
        const candidateById = new Map(candidates.data.map(candidate => [String(candidate.candidate_id), candidate])); const savedRows = rows.map(row => ({ ...row, business_id: business.id, evaluation_run_id: evalRun.id, decision_run_id: req.params.id, evaluation_version: SLICE_B_EVALUATION_VERSION, attributed_target_resources: row.attributed_target_resources || [], evidence_refs: candidateById.get(String(row.candidate_id))?.evidence_refs || [], interpretation_input_hash: candidateById.has(String(row.candidate_id)) ? interpretationInputHash(candidateById.get(String(row.candidate_id)), evidence.packet) : null, model_provider: response.provider || "openai", model_name: response.model || provider.model, instruction_version: INSTRUCTION_VERSION, provider_response_id: response.response_id || null, input_tokens: Number(response.usage?.prompt_tokens || response.usage?.input_tokens || 0), output_tokens: Number(response.usage?.completion_tokens || response.usage?.output_tokens || 0), limitations: row.limitations || [], overlap_group_id: row.overlap_group_id || null }));
      ...
      onBatchFailure: async ({ batchIndex, error, response, model }) => { const claim = ownedBatchClaims.get(batchIndex); if (claim) { const metadata = response || error?.providerMetadata || {}; const usage = metadata.usage || {}; const unknownOutcome = error?.code === "PROVIDER_OUTCOME_UNKNOWN"; const failedCost = unknownOutcome ? { cost_usd: null, cost_status: "unknown" } : calculateConfiguredCost({ inputTokens: Number(usage.prompt_tokens || usage.input_tokens || 0), outputTokens: Number(usage.completion_tokens || usage.output_tokens || 0), pricing: configuredModelPricing(process.env, model || metadata.model || provider.model) }); const failure = await admin.rpc("record_candidate_interpretation_failure", { p_batch_id: claim.batchId, p_claim_token: claim.claimToken, p_error_code: String(error?.code || error?.message || "PROVIDER_FAILURE").replace(/[^A-Z0-9_]/gi, "_").slice(0, 80), p_response_id: metadata.response_id || null, p_model: model || metadata.model || null, p_input_tokens: Number(usage.prompt_tokens || usage.input_tokens || 0), p_output_tokens: Number(usage.completion_tokens || usage.output_tokens || 0), p_cost_usd: failedCost.cost_usd, p_cost_status: failedCost.cost_status, p_elapsed_ms: claim.startedAt ? Date.now() - claim.startedAt : null }); if (failure.error || failure.data !== true) throw failure.error || new Error("BATCH_FAILURE_NOT_RECORDED"); } },
      onRetry: async () => process.env.V105_RECOVERY_MODE === "1" ? false : true
    });
    ...
    const rows = result.rows.map(row => { const { reason_codes: _providerReasonCodes, ...persisted } = row; return { ...persisted, business_id: business.id, evaluation_run_id: evalRun.id, decision_run_id: req.params.id, evaluation_version: SLICE_B_EVALUATION_VERSION, attributed_target_resources: row.attributed_target_resources || [], evidence_refs: candidateById.get(row.candidate_id)?.evidence_refs || [], interpretation_input_hash: candidateById.has(row.candidate_id) && row.interpretation_state === "complete" ? interpretationInputHash(candidateById.get(row.candidate_id), evidence.packet) : null, model_provider: result.modelProvider, model_name: result.modelName, instruction_version: INSTRUCTION_VERSION, limitations: row.limitations || [], overlap_group_id: row.overlap_group_id || null }; });
    ...
    for (const row of result.rows) {
      const status = row.deterministic_disposition === "reject" || ["reject_mismatch", "reject_wrong_page_type", "reject_overlap_redundant"].includes(row.interpretive_disposition) ? "rejected" : row.interpretation_state === "complete" ? "interpreted" : row.deterministic_disposition === "pass" ? "eligible" : "discovered";
