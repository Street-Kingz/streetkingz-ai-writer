  ...
  try {
    ...
    const result = await evaluateCandidates({ candidates: candidates.data, packet: evidence.packet, interpretationProvider: provider,
      resolveBatch: async ({ batch, batchIndex, inputHash }) => {
        ...
        const claimToken = randomUUID(); const claim = await admin.rpc("claim_candidate_interpretation_batch", { p_batch_id: found.data.id, p_claim_token: claimToken, p_timeout_seconds: 300 }); if (claim.error) throw claim.error; if (!claim.data) { if (found.data.outcome_state === "unknown_provider_outcome") { const error = new Error("PROVIDER_OUTCOME_UNKNOWN"); error.code = "PROVIDER_OUTCOME_UNKNOWN"; throw error; } return { pending: true }; } const started = await admin.rpc("begin_candidate_interpretation_attempt", { p_batch_id: found.data.id, p_claim_token: claimToken }); if (started.error || !started.data) throw started.error || new Error("BATCH_ATTEMPT_CLAIM_FAILED"); ownedBatchClaims.set(batchIndex, { claimToken, batchId: found.data.id }); return null;
      },
      ...
      onBatchFailure: async ({ batchIndex, error, response, model }) => { const claim = ownedBatchClaims.get(batchIndex); if (claim) { const usage = response?.usage || {}; const failedCost = calculateConfiguredCost({ inputTokens: Number(usage.prompt_tokens || usage.input_tokens || 0), outputTokens: Number(usage.completion_tokens || usage.output_tokens || 0), pricing: configuredModelPricing(process.env, model || provider.model) }); await admin.rpc("record_candidate_interpretation_failure", { p_batch_id: claim.batchId, p_claim_token: claim.claimToken, p_error_code: String(error?.code || error?.message || "PROVIDER_FAILURE").replace(/[^A-Z0-9_]/gi, "_").slice(0, 80), p_response_id: response?.response_id || null, p_model: model || null, p_input_tokens: Number(usage.prompt_tokens || usage.input_tokens || 0), p_output_tokens: Number(usage.completion_tokens || usage.output_tokens || 0), p_cost_usd: failedCost.cost_usd, p_cost_status: failedCost.cost_status }); } },
