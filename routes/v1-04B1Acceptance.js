...
const allowedOrigins = (origin, host) => { if (!origin) return true; try { const url = new URL(origin); return ["http:", "https:"].includes(url.protocol) && url.host === host && /^(?:127\.0\.0\.1|localhost)(?::\d+)?$/.test(url.hostname); } catch { return false; } };
...
export function createV104B1AcceptanceRouter({ enabled = false, bootstrapToken = crypto.randomBytes(32).toString("base64url"), config = () => productKernelConfig(process.env, { privileged: true }) } = {}) {
  ...
  const peerGuard = (req, res, next) => isLoopbackPeer(req.socket?.remoteAddress) && allowedHost(req.get("host")) && allowedOrigins(req.get("origin"), req.get("host")) ? next() : res.status(404).end();
  ...
  router.post("/internal/v1-04/session/cleanup", async (req, res) => {
    ...
    const businesses = account.data ? await admin.from("businesses").select("id").eq("account_id", account.data.id) : { data: [], error: null }; if (businesses.error) return res.status(503).json({ error: { code: "LOCAL_SESSION_CLEANUP_FAILED", message: "Local acceptance session could not be cleaned up." } });
    for (const business of businesses.data || []) { const connections = await admin.from("connections").select("id,provider_type").eq("business_id", business.id); if (connections.error) return res.status(503).json({ error: { code: "LOCAL_SESSION_CLEANUP_FAILED", message: "Local acceptance session could not be cleaned up." } }); for (const connection of connections.data || []) { if (connection.provider_type === "google_search_console") await admin.rpc("gsc_disconnect", { p_business_id: business.id, p_connection_id: connection.id }); const attempts = await admin.from(connection.provider_type === "woocommerce" ? "woocommerce_auth_attempts" : "gsc_oauth_attempts").select(connection.provider_type === "woocommerce" ? "credential_reference" : "staged_secret_reference").eq("connection_id", connection.id); for (const attempt of attempts.data || []) { const ref = attempt.credential_reference || attempt.staged_secret_reference; if (ref) await admin.rpc("vault_delete_secret", { secret_id: ref }); } } }
