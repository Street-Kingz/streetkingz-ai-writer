...
test("B1 real route/Vault lifecycle uses authenticated local tenant and fake Google", { skip: !enabled }, async t => {
  ...
  let exchanges = 0, activeRefreshFailure = false, probedSiteUrl = null;
  setGoogleSearchConsoleTransportFactory(() => ({
    ...
    async accessToken() { if (activeRefreshFailure) { const error = new Error("invalid_grant"); error.code = "GSC_REAUTH_REQUIRED"; throw error; } return "fake-access-token"; },
    ...
    async site(_token, siteUrl) { probedSiteUrl = siteUrl; return { siteUrl, permissionLevel: siteUrl.startsWith("sc-domain:") ? "siteFullUser" : "siteOwner" }; }
  }));
  ...
  const source = await admin.from("organic_evidence_sources").select("id").eq("business_id", business.data.id).eq("source_kind", "search_console").single(); assert.ifError(source.error);
  const run = await admin.rpc("organic_begin_run", { p_source_id: source.data.id, p_retrieved_at: new Date().toISOString() }); assert.ifError(run.error); const evidenceAsOf = new Date(Date.now() - 1000).toISOString(); const finished = await admin.rpc("organic_finish_run", { p_run_id: run.data.id, p_state: "complete", p_completeness_state: "complete", p_evidence_as_of: evidenceAsOf }); assert.ifError(finished.error);
  activeRefreshFailure = true; const reauth = await request(server, "POST", "/api/product/organic-evidence/search-console/reauth-check", token, { connection_id: connectionId }); assert.equal(reauth.status, 409); assert.equal(reauth.body.error.code, "GSC_REAUTH_REQUIRED"); const reauthStatus = await request(server, "GET", `/api/product/organic-evidence/search-console/status?connection_id=${connectionId}`, token); assert.equal(reauthStatus.body.connection_state, "reauthentication_required"); assert.equal(reauthStatus.body.has_current_complete_evidence, true); assert.equal(new Date(reauthStatus.body.evidence_as_of).toISOString(), new Date(evidenceAsOf).toISOString());
  ...
  assert.deepEqual(wooAfter.data, wooBefore.data); assert.deepEqual(businessAfter.data, businessBefore.data); assert.equal(exchanges, 1); assert.equal((await admin.from("gsc_connections").select("connection_state").eq("connection_id", connectionId).single()).data.connection_state, "disconnected"); const stale = await admin.from("organic_evidence_sources").select("evidence_state,current_complete_run,evidence_as_of").eq("id", source.data.id).single(); assert.equal(stale.data.evidence_state, "stale"); assert.equal(stale.data.current_complete_run, run.data.id); assert.equal(new Date(stale.data.evidence_as_of).toISOString(), new Date(evidenceAsOf).toISOString()); assert.ok(store.data.id);
});
