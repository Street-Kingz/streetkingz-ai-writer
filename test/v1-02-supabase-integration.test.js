...
test("V1-02 hardened real Supabase boundary", { skip: !enabled }, async t => {
  ...
  const wooConnection = (await admin.from("connections").insert({ business_id: a.business.id, provider_type: "woocommerce" }).select().single()).data;
  const wooRpcBypass = await a.caller.rpc("product_transition_connection", { p_connection_id: wooConnection.id, p_status: "connected", p_consent_state: "granted", p_correlation_id: crypto.randomUUID() }); assert.ok(wooRpcBypass.error); assert.match(wooRpcBypass.error.message, /WOO_CONNECTION_STATE_MANAGED/);
  const wooPatchBypass = await request(a.token, "PATCH", `/api/product/connections/${wooConnection.id}`, { status: "connected", consent_state: "granted" }); assert.equal(wooPatchBypass.status, 409); assert.equal(wooPatchBypass.body.error.code, "WOO_CONNECTION_STATE_MANAGED");
  const wooAttemptUser = `v102-woo-${crypto.randomUUID()}`; const wooAttempt = await admin.rpc("woo_create_auth_attempt", { p_user_id: wooAttemptUser, p_account_id: a.account.id, p_business_id: a.business.id, p_connection_id: wooConnection.id, p_canonical_base_url: "https://woo.example/wp", p_expires_at: new Date(Date.now() + 60000).toISOString() }); assert.ifError(wooAttempt.error); assert.ifError((await admin.rpc("woo_claim_auth_attempt", { p_user_id: wooAttemptUser })).error); const wooCapture = await admin.rpc("woo_capture_callback", { p_attempt_id: wooAttempt.data, p_consumer_key: "ck_synthetic", p_consumer_secret: "cs_synthetic", p_key_permissions: "read" }); assert.ifError(wooCapture.error); const wooComplete = await admin.rpc("woo_complete_connection", { p_attempt_id: wooAttempt.data, p_home_url: "https://woo.example/", p_site_url: "https://woo.example/wp", p_version: "9", p_timezone: "UTC", p_currency: "GBP", p_correlation_id: crypto.randomUUID() }); assert.ifError(wooComplete.error); const wooStore = (await admin.from("commerce_stores").select("id").eq("id", wooComplete.data).single()).data; const wooSecretReference = (await admin.from("connections").select("secret_reference").eq("id", wooConnection.id).single()).data.secret_reference;
  const wooDisconnect = await request(a.token, "PATCH", `/api/product/connections/${wooConnection.id}`, { status: "disconnected", consent_state: "revoked" }); assert.equal(wooDisconnect.status, 200); assert.equal((await admin.rpc("vault_read_secret", { secret_id: wooSecretReference })).data, null); assert.equal((await admin.from("commerce_stores").select("sync_state").eq("id", wooStore.id).single()).data.sync_state, "stale");
