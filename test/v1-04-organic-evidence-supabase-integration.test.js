...
test("Slice A local Supabase source/run lifecycle, RLS and LKG", { skip: !enabled }, async t => {
  ...
  const wrongBusinessConnection = await admin.rpc("organic_ensure_source", { p_business_id: business.data.id, p_source_class: "customer_connected", p_source_kind: "search_console", p_provider_id: "google_search_console", p_connection_id: connectionB });
  assert.ok(wrongBusinessConnection.error, "Business A cannot bind Business B Connection");
  assert.match(wrongBusinessConnection.error.message, /organic_source_connection_business_fk|foreign key/i);
  const crossBound = await admin.rpc("organic_ensure_source", { p_business_id: otherBusiness.data.id, p_source_class: "customer_connected", p_source_kind: "search_console", p_provider_id: "google_search_console", p_connection_id: connectionA });
  assert.ok(crossBound.error, "Business B cannot bind Business A Connection");
  assert.match(crossBound.error.message, /organic_source_connection_business_fk|foreign key/i);
  ...
  const otherBound = await admin.rpc("organic_ensure_source", { p_business_id: otherBusiness.data.id, p_source_class: "customer_connected", p_source_kind: "search_console", p_provider_id: "google_search_console", p_connection_id: connectionB });
  assert.ifError(otherBound.error);
