...
test("Slice D real local schema persists derived Keyword Ideas/SERP evidence and deduplicates", { skip: !enabled }, async t => {
  ...
  const request = await admin.from("organic_external_provider_requests").insert({ business_id: business.data.id, source_id: source.data.id, run_id: run.run_id, seed_id: seed.data.id, endpoint: "/v3/dataforseo_labs/google/keyword_ideas/live", request_class: "keyword_ideas", request_fingerprint: "fixture-cost-fingerprint", scope: { location_code: 2826 }, status: "running", estimated_reserved_cost: 0.0144, cache_state: "miss", started_at: now }).select("id").single(); assert.ifError(request.error);
  assert.ifError((await admin.from("organic_external_provider_requests").update({ actual_cost: 0.0144, provider_task_id: "synthetic-task" }).eq("id", request.data.id)).error);
  ...
  assert.ifError((await admin.from("organic_external_provider_requests").update({ status: "failed", error_code: "EXTERNAL_OBSERVATION_PERSIST_FAILED", completed_at: now }).eq("id", request.data.id)).error);
  const failedRequest = await admin.from("organic_external_provider_requests").select("actual_cost,provider_task_id,status,error_code").eq("id", request.data.id).single(); assert.ifError(failedRequest.error);
  assert.deepEqual(failedRequest.data, { actual_cost: 0.0144, provider_task_id: "synthetic-task", status: "failed", error_code: "EXTERNAL_OBSERVATION_PERSIST_FAILED" });
  ...
  const forbiddenDirect = await admin.from("organic_external_observations").insert({ ...keyword.rows[0], observation_identity: "direct-observation-forbidden", direct_or_derived: "direct" });
  assert.equal(forbiddenDirect.error?.code, "23514");
