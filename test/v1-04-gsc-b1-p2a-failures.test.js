...
test("P2-A OAuth, transport, Vault and provider failure ledger", { skip: !enabled }, async t => {
  ...
  await t.test("P2-OAUTH-013 code and error deterministically rejects before claim and exchange", async () => { const s = await start(); const before = exchanges; const r = await callback(s.state, "code=x&error=access_denied"); assert.equal(r.status, 400); assert.equal(r.body.error.code, "GSC_CALLBACK_INVALID"); assert.equal(exchanges, before); });
