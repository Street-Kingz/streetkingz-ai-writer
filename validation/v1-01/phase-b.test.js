...
import { buildCandidates, sanitiseCandidates, sha256, validateCandidates, captureCommercialSnapshot, RUBRIC, PUBLIC_DIR, PRIVATE_DIR, assertNoDecisionArtifacts } from "./index.js";
...
test("private path is ignored and public bundle has no commercial fields", () => { const ignored = execFileSync("git", ["check-ignore", "artifacts/private/v1-01/probe.json"], { encoding: "utf8" }); assert.ok(ignored); const text = fs.readFileSync(`${PUBLIC_DIR}/candidate-universe.sanitised.json`, "utf8"); assert.doesNotMatch(text, /units_sold|revenue|sales_velocity|current_stock|cogs|margin/i); });
test("freeze outputs preserve exact dates and missing reliability statuses", () => { const readiness = JSON.parse(fs.readFileSync(`${PUBLIC_DIR}/data-readiness.sanitised.json`)); assert.equal(readiness.status, "BLOCKED"); assert.match(fs.readFileSync(`${PRIVATE_DIR}/commercial-snapshot.json`, "utf8"), /missing/); assert.match(fs.readFileSync(`${PRIVATE_DIR}/commercial-snapshot.json`, "utf8"), /2026-08-23/); });
