...
begin
  ...
  select cn.* into v_connection from public.connections cn join public.businesses b on b.id=cn.business_id
    where cn.business_id=p_business_id and b.account_id=p_account_id and b.status='active'
      and cn.provider_type='google_search_console' for update;
  ...
  update public.gsc_oauth_attempts set status='superseded', consumed_at=clock_timestamp()
    where connection_id=v_connection.id and status in ('pending','processing');
  delete from vault.secrets where id in (select staged_secret_reference from public.gsc_oauth_attempts where connection_id=v_connection.id and status='superseded' and staged_secret_reference is not null);
