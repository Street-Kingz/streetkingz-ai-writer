...
begin
  ...
  update public.gsc_oauth_attempts oa set status='superseded', consumed_at=clock_timestamp() where oa.connection_id=v_connection.id and oa.status in ('pending','processing');
  delete from vault.secrets where id in (select oa.staged_secret_reference from public.gsc_oauth_attempts oa where oa.connection_id=v_connection.id and oa.status='superseded' and oa.staged_secret_reference is not null);
  insert into public.gsc_oauth_attempts(account_id,business_id,connection_id,state_hash,pkce_verifier,expires_at) values(p_account_id,p_business_id,v_connection.id,p_state_hash,p_pkce_verifier,p_expires_at) returning * into v_attempt;
