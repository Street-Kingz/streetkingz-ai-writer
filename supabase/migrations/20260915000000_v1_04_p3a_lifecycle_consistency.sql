-- P3-A lifecycle consistency. Existing migrations remain immutable.
...
begin
  ...
  update public.gsc_connections g
    ...
    where g.connection_id=p_connection_id and g.business_id=p_business_id
      ...
      and (p_expected_secret_reference is null or c.secret_reference=p_expected_secret_reference)
      and c.status='connected';
  if not found then raise exception 'GSC_REAUTHENTICATION_STALE'; end if;
end $$;
revoke all on function public.gsc_mark_reauthentication_required(uuid,uuid), public.gsc_mark_reauthentication_required(uuid,uuid,uuid) from public, anon, authenticated;
grant execute on function public.gsc_mark_reauthentication_required(uuid,uuid), public.gsc_mark_reauthentication_required(uuid,uuid,uuid) to service_role;
...
begin
  ...
  if old_site is not null and old_site is distinct from p_site_url then
    update public.organic_evidence_sources set current_complete_run=null,current_completeness_state=null,last_successful_at=null,evidence_as_of=null,evidence_state='never_collected',updated_at=clock_timestamp()
      where id=result.id;
  end if;
