...
create function public.woo_deny_auth_attempt(p_user_id text) returns boolean language plpgsql security definer set search_path='' as $$ declare a public.woocommerce_auth_attempts;begin select * into a from public.woocommerce_auth_attempts where user_id=p_user_id and status='pending' and expires_at>clock_timestamp() for update;if not found then return false;end if;update public.woocommerce_auth_attempts set status='denied',safe_error_code='MERCHANT_DENIED' where id=a.id;insert into public.audit_events(account_id,business_id,event_type,safe_metadata) values(a.account_id,a.business_id,'woocommerce_authorisation_denied',jsonb_build_object('attempt_id',a.id));return true;end $$;
...
revoke all on function public.woo_create_auth_attempt(text,uuid,uuid,uuid,text,timestamptz),public.woo_claim_auth_attempt(text),public.woo_capture_callback(uuid,text,text,text),public.woo_abort_auth_attempt(uuid,text),public.woo_record_verification_retryable(uuid,text),public.woo_cleanup_expired_attempts(),public.woo_complete_connection(uuid,text,text,text,text,text,uuid),public.woo_fail_connection(uuid,text),public.woo_deny_auth_attempt(text),public.commerce_promote_generation(uuid,bigint) from public,anon,authenticated;
grant execute on function public.woo_create_auth_attempt(text,uuid,uuid,uuid,text,timestamptz),public.woo_claim_auth_attempt(text),public.woo_capture_callback(uuid,text,text,text),public.woo_abort_auth_attempt(uuid,text),public.woo_record_verification_retryable(uuid,text),public.woo_cleanup_expired_attempts(),public.woo_complete_connection(uuid,text,text,text,text,text,uuid),public.woo_fail_connection(uuid,text),public.woo_deny_auth_attempt(text),public.commerce_promote_generation(uuid,bigint) to service_role;
