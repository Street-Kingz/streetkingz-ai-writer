-- V1-03 lifecycle correction: one Connection-scoped lock serialises disconnect and verification.
...
declare v_account public.accounts; v_row public.connections; v_allowed boolean; r record;
begin
  ...
  if p_status='disconnected' and v_row.provider_type='woocommerce' then
    for r in select id,account_id,business_id,credential_reference from public.woocommerce_auth_attempts where connection_id=v_row.id and status in ('pending','claimed','callback_received') for update loop
      if r.credential_reference is not null then begin delete from vault.secrets where id=r.credential_reference; exception when others then raise exception 'SECRET_OPERATION_FAILED'; end; end if;
      update public.woocommerce_auth_attempts set status='superseded',credential_reference=null,safe_error_code='CONNECTION_DISCONNECTED' where id=r.id;
      insert into public.audit_events(account_id,business_id,event_type,correlation_id,safe_metadata) values(r.account_id,r.business_id,'woocommerce_auth_attempt_cancelled',p_correlation_id::text,jsonb_build_object('attempt_id',r.id,'reason','connection_disconnected'));
    end loop;
  end if;
...
declare a public.woocommerce_auth_attempts;s public.commerce_stores;old_ref uuid;v_connection public.connections;v_connection_id uuid;
begin
  select connection_id into v_connection_id from public.woocommerce_auth_attempts where id=p_attempt_id;
  ...
  if found then
    if s.canonical_base_url<>a.canonical_base_url or s.connection_id<>a.connection_id then raise exception 'WOO_STORE_REBIND_DENIED'; end if;
  ...
  select * into v_connection from public.connections where id=a.connection_id for update;
  old_ref:=v_connection.secret_reference;
...
create or replace function public.woo_fail_connection(p_attempt_id uuid,p_safe_error_code text) returns void language plpgsql security definer set search_path='' as $$
declare a public.woocommerce_auth_attempts;v_connection public.connections;old_ref uuid;v_connection_id uuid;
begin
  select connection_id into v_connection_id from public.woocommerce_auth_attempts where id=p_attempt_id;
  if not found then return; end if;
  perform pg_advisory_xact_lock(hashtextextended(v_connection_id::text,0));
  select * into a from public.woocommerce_auth_attempts where id=p_attempt_id for update;
  if not found or a.status<>'callback_received' then return; end if;
  select * into v_connection from public.connections where id=a.connection_id for update;
  old_ref:=v_connection.secret_reference;
  if v_connection.status='disconnected' and v_connection.consent_state='revoked' then
    if a.credential_reference is not null then delete from vault.secrets where id=a.credential_reference; end if;
    update public.woocommerce_auth_attempts set status='superseded',credential_reference=null,safe_error_code='CONNECTION_DISCONNECTED' where id=a.id;
    return;
  end if;
  ...
  if a.credential_reference is not null and a.credential_reference<>old_ref then delete from vault.secrets where id=a.credential_reference; end if;
  ...
  update public.connections set status='error',consent_state='pending',safe_error_code=p_safe_error_code,safe_error_message='WooCommerce connection could not be verified.' where id=a.connection_id and old_ref is null and status<>'connected';
  update public.businesses set connection_status='error' where id=a.business_id and connection_status<>'connected';
end $$;
