...
export function sensitiveProductPath(req) { return /^\/api\/product\/(account|business|connections|decision-runs\/discover|decision-runs\/[0-9a-f-]{36}\/evaluate|woocommerce\/(authorize|verify|sync)|organic-evidence\/(search-console\/(connect|reconnect|select|disconnect|acquire)|site\/acquire|external\/acquire))/.test(req.path); }
export function expensiveProductPath(req) { return /\/((woocommerce\/sync)|(organic-evidence\/((site|external)\/acquire|search-console\/acquire))|(decision-runs\/[0-9a-f-]{36}\/evaluate))$/.test(req.path); }
