...
async function performWooRequest(origin,path,{fields=[],credentials,lookup=dns.lookup,request=https.request,timeoutMs=TIMEOUT_MS,maxBytes=MAX_BYTES,maxRedirects=MAX_REDIRECTS}={},operation){
 ...
 const expected=first.url;let target=appendStorePath(expected,path),validation=first;if(fields.length)target.searchParams.set('_fields',fields.join(','));
 for(const key of target.searchParams.keys())if(key!=='_fields')throw invalid();
 for(let redirects=0;;redirects++){
  throwIfCancelled(operation);const checked=validation;if(checked.url.hostname!==expected.hostname||!withinBase(checked.url.pathname,expected.pathname))throw invalid();for(const key of target.searchParams.keys())if(key!=='_fields')throw invalid();const pinned=checked.addresses[0];throwIfCancelled(operation);
