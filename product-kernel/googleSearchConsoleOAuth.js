...
export const GSC_REQUEST_TIMEOUT_MS = 15000;
export const GSC_RESPONSE_MAX_BYTES = 1024 * 1024;
...
export function publicSuffixRegistrableDomain(host) {
  ...
  const domain = getDomain(host.toLowerCase().replace(/\.$/, ""), { allowPrivateDomains: false });
  return domain || null;
}
...
export function propertyProbeMatches(requestedSiteUrl, providerResponse) {
  return typeof requestedSiteUrl === "string" && typeof providerResponse?.siteUrl === "string" && providerResponse.siteUrl === requestedSiteUrl;
}
export function createGscTransport({ fetchImpl = globalThis.fetch, env = process.env } = {}) {
  const c=config(env); const call=async(url, options={})=>{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),GSC_REQUEST_TIMEOUT_MS);try{const r=await fetchImpl(url,{...options,signal:controller.signal});if(!r.body)throw new ProductError("GSC_PROVIDER_MALFORMED","Google Search Console returned an invalid response.",502);const reader=r.body.getReader();const chunks=[];let size=0;while(true){const part=await reader.read();if(part.done)break;size+=part.value.byteLength;if(size>GSC_RESPONSE_MAX_BYTES){await reader.cancel();throw new ProductError("GSC_RESPONSE_TOO_LARGE","Google Search Console response exceeded the bounded limit.",502);}chunks.push(part.value);}const text=new TextDecoder().decode(Buffer.concat(chunks));let body;try{body=JSON.parse(text);}catch{throw new ProductError("GSC_PROVIDER_MALFORMED","Google Search Console returned malformed JSON.",502);}if(!r.ok) throw new ProductError(r.status===401||body?.error==="invalid_grant"?"GSC_REAUTH_REQUIRED":r.status===403?"GSC_PROPERTY_INVALID":r.status===429?"GSC_RATE_LIMITED":"GSC_PROVIDER_ERROR","Google Search Console request failed.",r.status===401||body?.error==="invalid_grant"||r.status===403?409:502);return body;}catch(e){if(e instanceof ProductError)throw e;throw new ProductError("GSC_PROVIDER_ERROR","Google Search Console request failed.",502);}finally{clearTimeout(timer);}};
  return { config:c, authorizationUrl({state,verifier}) { const u=new URL(AUTH_URL); u.search=new URLSearchParams({client_id:c.clientId,redirect_uri:c.callbackUrl,response_type:"code",scope:GSC_SCOPE,access_type:"offline",prompt:"consent",include_granted_scopes:"false",state,code_challenge:hash(verifier),code_challenge_method:"S256"}); return u.toString(); }, async exchangeCode(code,verifier){const r=await call(TOKEN_URL,{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:new URLSearchParams({code,client_id:c.clientId,client_secret:c.clientSecret,redirect_uri:c.callbackUrl,grant_type:"authorization_code",code_verifier:verifier}).toString()});if(typeof r.refresh_token!=="string"||!r.refresh_token)throw new ProductError("GSC_REFRESH_TOKEN_REQUIRED","Google did not return durable offline authorization.",409);const scopes=typeof r.scope === "string" ? r.scope.split(/\s+/).filter(Boolean) : null;if(!scopes || scopes.length !== 1 || scopes[0] !== GSC_SCOPE)throw new ProductError("GSC_SCOPE_INVALID","Google authorization did not grant the required read-only scope.",409);return r;}, async accessToken(refreshToken){const r=await call(TOKEN_URL,{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:new URLSearchParams({refresh_token:refreshToken,client_id:c.clientId,client_secret:c.clientSecret,grant_type:"refresh_token"}).toString()});if(typeof r.access_token!=="string")throw new ProductError("GSC_REAUTH_REQUIRED","Google authorization requires reconnection.",409);return r.access_token;}, async sitesList(accessToken){return call(`${API_URL}/sites`,{headers:{authorization:`Bearer ${accessToken}`}});}, async site(accessToken,siteUrl){return call(`${API_URL}/sites/${encodeURIComponent(siteUrl)}`,{headers:{authorization:`Bearer ${accessToken}`}})}, async searchAnalytics(accessToken,siteUrl,body){return call(`${API_URL}/sites/${encodeURIComponent(siteUrl)}/searchAnalytics/query`,{method:"POST",headers:{authorization:`Bearer ${accessToken}`,"content-type":"application/json"},body:JSON.stringify(body)});} };
}
const defaultTransportFactory = options => createGscTransport(options);
let transportFactory = defaultTransportFactory;
export function setGoogleSearchConsoleTransportFactory(factory) { transportFactory = factory || defaultTransportFactory; }
