...
export async function loadDiscoveryEvidence({ admin, businessId }) {
  ...
  const fingerprintInput = { business_id: business.id, commerce: { store_id: store?.id, generation_id: generation?.id }, site: { selected_run_id: siteRun?.id, latest_attempt_id: siteLatest?.id, state: packet.site.state }, search_console: { selected_run_id: gscRun?.id, state: packet.search_console.state }, external: { selected_run_id: externalRun?.id, latest_attempt_id: externalLatest?.id, state: packet.external.state, supported_locale: packet.external.supported_locale }, limitations: [...packet.site.limitations, ...(packet.external.limitations || [])] };
