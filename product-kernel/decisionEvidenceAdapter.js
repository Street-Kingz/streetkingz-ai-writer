...
export async function loadDiscoveryEvidence({ admin, businessId }) {
  ...
  const packet = {
    ...
    external: { state: externalRun?.state === "complete" ? "available" : externalLatest?.state === "partial" ? "partial" : "missing", selected_run_id: externalRun?.id || externalLatest?.id || null, supported_locale: { market: "GB", language: "en" }, rows: Array.from(externalRows.reduce((groups, row) => { const key = String(row.seed_id) + ":" + String(row.query_text); let group = groups.get(key); if (!group) { group = { query: row.query_text, market: business.primary_market || null, language: business.primary_language || null, search_volume: row.search_volume, observed_at: row.observed_at, source_record_ids: [], source_run_or_generation_reference: String(externalLatest?.id || ""), serp: [] }; groups.set(key, group); } group.source_record_ids.push(String(row.id)); if (row.observation_type === "serp_organic_result") group.serp.push({ rank: row.rank_absolute || row.rank_group, url: row.result_url, domain: row.result_domain, title: row.result_title, description: row.result_description, source_record_id: String(row.id) }); return groups; }, new Map()).values()).map(group => ({ ...group, source_record_id: group.source_record_ids[0] || null })) }
  };
