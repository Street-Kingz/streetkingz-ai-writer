...
export async function loadDiscoveryEvidence({ admin, businessId }) {
  ...
  const packet = {
    ...
    commerce: { state: store?.sync_state === "complete" && generation?.state === "complete" ? "available" : "unavailable", store_id: store?.id || null, generation_id: generation?.id || null, products, categories, relations: links },
    ...
    search_console: { state: gscRun?.state === "complete" ? "available" : "missing", selected_run_id: gscRun?.id || null, rows: gscRows.map(row => ({ ...row, page_id: row.page_url ? pageIdsByUrl.get(row.page_url) || null : null })) },
    external: { state: externalRun?.state === "complete" ? "available" : externalLatest?.state === "partial" ? "partial" : "missing", selected_run_id: externalRun?.id || null, rows: Array.from(externalRows.reduce((groups, row) => { const key = `${row.seed_id}:${row.query_text}`; let group = groups.get(key); if (!group) { group = { query: row.query_text, market: "GB", language: "en", search_volume: row.search_volume, observed_at: row.observed_at, serp: [] }; groups.set(key, group); } if (row.observation_type === "serp_organic_result") group.serp.push({ rank: row.rank_absolute || row.rank_group, url: row.result_url, domain: row.result_domain, title: row.result_title, description: row.result_description }); return groups; }, new Map()).values()) }
  };
