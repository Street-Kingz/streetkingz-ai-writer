...
export function buildBatchIdentity({ candidates, packet }) { return evaluationHash({ evaluation_version: SLICE_B_EVALUATION_VERSION, filter_version: FILTER_VERSION, interpretation_version: INTERPRETATION_VERSION, instruction_version: INSTRUCTION_VERSION, candidate_ids: candidates.map(c => String(c.candidate_id)).sort(), candidate_input_hashes: candidates.map(c => evaluationHash(buildInterpretationRequest({ candidate: c, packet }).input)).sort() }); }
